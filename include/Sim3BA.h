...
struct ReprojCostShape {
    ReprojCostShape(int jid,
                   double u_obs, double v_obs,
                   double fx, double fy, double cx, double cy,
                   const std::vector<int>& parent,
                   const std::vector<Eigen::Vector3d>& offset,
...
OptimizePoseShapeReprojection(const ark::AvatarModel& model,
                                  ark::Avatar& avatar,
                                  ...
                                  const PosePriorAAAnalytic::GaussianMixture* gmmPosePrior = nullptr)
{
    ...
    for (const PixelKP& kp : kps) {
        ...
        auto* reprojFunctor = new ReprojCostShape(jid, kp.u, kp.v, fx, fy, cx, cy,
                                                parent, offset, R0_init, nJ, jsRegPtr);
        auto* cost = new ceres::DynamicAutoDiffCostFunction<ReprojCostShape>(reprojFunctor);
                         ...
                         const PosePriorAAAnalytic::GaussianMixture* gmmPosePrior = nullptr)
{
    ...
    for (const auto& kp : kps) {
        ...
        auto* functor = new ReprojCost(jid, kp.u, kp.v, fx, fy, cx, cy, parent, offset, R0_init, nJ);
        auto* cost = new ceres::DynamicAutoDiffCostFunction<ReprojCost>(functor);
